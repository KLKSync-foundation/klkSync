...
// File: interfaces/IsKSP.sol


pragma solidity >=0.7.5;


interface IsKSP is IERC20 {
...
interface IgOHM is IERC20 {
  ...
  function migrate( address _staking, address _sKSP ) external;
}
...
contract sKSP is IsKSP, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    constructor() ERC20("Staked OHM", "sKSP", 9) ERC20Permit("Staked OHM") {
    ...
    // this function is called by the treasury, and informs sKSP of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sKSP
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sKSP: insufficient balance");
    }
    ...
    // toG converts an sKSP balance to gOHM terms. gOHM is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gOHM.balanceTo(amount);
    }

    // fromG converts a gOHM balance to sKSP terms. sKSP is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gOHM.balanceFrom(amount);
    }

    // Staking contract holds excess sKSP
