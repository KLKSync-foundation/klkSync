...
abstract contract ERC20 is IERC20 {
    ...
    //temp1
    function _mint_11111(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");
        _beforeTokenTransfer(address(0), account, amount);
        _totalSupply = _totalSupply.add(amount);
        _balances[account] = _balances[account].add(amount);
...
contract KSPERC20Token is ERC20Permit, IKSP, VaultOwned {
    using SafeMath for uint256;

    constructor() ERC20("KSP", "KSP", 9) ERC20Permit("KSP") {}
    ...
    function burn(uint256 amount) external override(IERC20, IKSP) {
...
contract KSPWithPremium is KSPERC20Token {
    using SafeMath for uint256;
